...
def cmap_density():
    return LinearSegmentedColormap.from_list(
        "density_basin",
        ["#05070b", "#071d3a", "#123c69", "#2b8cbe", "#a6cee3", "#f0f9ff"]
    )
...
def draw_centroid_arrows(ax, centroids):
    ...
    if "time" in df.columns and df["time"].astype(str).str.len().max() > 0:
        for state, sub in df.groupby("state"):
            ...
            color = STATE_COLORS.get(state, "#e5e7eb")
            ...
            for _, r in sub.iterrows():
                ...
                ax.text(r["x"], r["y"], str(r["time"]), color="white", fontsize=6,
                        ha="center", va="center", zorder=10)
    else:
        for _, r in df.iterrows():
            color = STATE_COLORS.get(r["state"], "#e5e7eb")
            ax.scatter(r["x"], r["y"], s=60, c=color, edgecolor="white", linewidth=0.7, zorder=9)
            ax.text(r["x"], r["y"], str(r["state"])[:10], color="white", fontsize=6,
                    ha="center", va="center", zorder=10)
...
def finalize_2d_ax(ax, title, xlabel="Latent coordinate 1", ylabel="Latent coordinate 2"):
    ...
    ax.set_xlabel(xlabel, color="#cbd5e1", fontsize=8)
    ax.set_ylabel(ylabel, color="#cbd5e1", fontsize=8)
...
def make_main_figure(Z, labels, land, centroids, vec_df, outbase, dpi=600):
    fig = plt.figure(figsize=(14.2, 10.2), facecolor="#05070b")
...
def make_3d_surface_figure(Z, labels, land, outbase, dpi=600):
    fig = plt.figure(figsize=(9.8, 7.6), facecolor="#05070b")
    ...
    ax.set_xlabel("Latent coordinate 1", color="#cbd5e1", labelpad=8)
    ax.set_ylabel("Latent coordinate 2", color="#cbd5e1", labelpad=8)
    ax.set_zlabel("Pseudo-energy U(z)", color="#cbd5e1", labelpad=8)

    ax.tick_params(colors="#cbd5e1", labelsize=7)
    ax.xaxis.pane.set_facecolor((0.02, 0.03, 0.05, 1.0))
    ax.yaxis.pane.set_facecolor((0.02, 0.03, 0.05, 1.0))
    ax.zaxis.pane.set_facecolor((0.02, 0.03, 0.05, 1.0))
