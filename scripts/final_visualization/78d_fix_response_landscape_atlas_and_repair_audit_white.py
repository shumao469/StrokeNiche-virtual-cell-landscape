...
# Source: E:\vir\ST\78d_fix_response_landscape_atlas_and_repair_audit.py
...
def plot_landscape(
    ...
    if label_axes:
        ...
        ax.set_xticklabels([])
        ax.set_yticklabels([])
    ax.grid(True, color="#e5e7eb", lw=0.35, alpha=0.55)
    return im
...
def make_atlas(
    ...
    fig = plt.figure(figsize=(18, max(3.2, 2.75 * nrows)), facecolor="#ffffff")
    ...
    fig.suptitle(title, fontsize=24, fontweight="bold", y=0.975, color="#1f2937")
    ...
    cb1.set_label("Core probability", color="#1f2937")
    cb1.ax.tick_params(colors="#1f2937")
    ...
    if include_mode == "main_rescue":
        ...
        cb2.set_label("Response priority", color="#1f2937")
        cb2.ax.tick_params(colors="#1f2937")
    else:
        cax2 = fig.add_subplot(gs[max(2, nrows // 2): nrows + 1, -1])
        norm = Normalize(vmin=0, vmax=1)
        sm = ScalarMappable(norm=norm, cmap="viridis")
...
def make_single_candidate_figure(
    ...
    fig = plt.figure(figsize=(15.5, 10.4), facecolor="#ffffff")
    gs = gridspec.GridSpec(
        2, 3,
    ...
    cb.set_label("Response priority" if not signed_audit else "Response/audit value", color="#1f2937")
