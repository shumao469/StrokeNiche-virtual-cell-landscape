...
# Source: E:\vir\ST\81b_surrogate_feature_response_audit_fixed.py
...
def add_linear_trend(ax, x, y):
    ...
    try:
        ...
        ax.plot(xs, coef[0] * xs + coef[1], color="#1f2937", lw=1.6, alpha=0.9)
    except Exception:
        return
...
def plot_scatter_grid(df: pd.DataFrame, outdir: str, label_col: str, fallback: bool) -> Tuple[Dict, pd.DataFrame]:
    ...
    for i, cand in enumerate(candidates):
        ...
        for j, metric in enumerate(metrics):
            ...
            ax.text(
                ...
                color="#1f2937", fontsize=8,
                bbox=dict(boxstyle="round,pad=0.2", fc="#f3f4f6", ec="none", alpha=0.75)
            )

            if i == 0:
                ax.set_title(titles[j], color="#1f2937", fontweight="bold", fontsize=12)

            if j == 0:
                ax.set_ylabel(cand, color="#1f2937", fontweight="bold")
            ax.set_xlabel("Candidate feature score", color="#1f2937")
    ...
    fig.suptitle(title, fontsize=19, color="#1f2937", fontweight="bold", y=0.985)
    ...
    fig.text(0.5, 0.025, note, ha="center", va="center", color="#374151", fontsize=8.5)
...
def plot_heatmap(summary: pd.DataFrame, outdir: str) -> Dict:
    ...
    ax.set_xticklabels(mat.columns, rotation=35, ha="right", color="#1f2937")
    ax.set_yticks(range(mat.shape[0]))
    ax.set_yticklabels(mat.index, color="#1f2937")

    for i in range(mat.shape[0]):
        for j in range(mat.shape[1]):
            val = mat.values[i, j]
            txt = "NA" if not np.isfinite(val) else f"{val:.2f}"
            ax.text(j, i, txt, ha="center", va="center", color="#1f2937", fontsize=8)

    ax.set_title("Feature-response Spearman correlation", color="#1f2937", fontweight="bold", pad=12)
    ax.tick_params(colors="#1f2937")
    for sp in ax.spines.values():
        sp.set_color("0.35")

    cb = fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
    cb.set_label("Spearman ρ", color="#1f2937")
    cb.ax.tick_params(colors="#1f2937")
