...
"""
Step75F | Color-consistent before/after virtual perturbation spatial rescue atlas
...
def make_a3_figure(meta, x, y, maps, outbase, point_size=2.0, dpi=600):
    ...
    fig.text(
        0.5,
        0.012,
        "Maps show visualization-level virtual perturbation/state-editing proxies; baseline and perturbed core maps share the same color scale. These are not wet-lab KO/blockade results or observed cell-fate transitions.",
        ha="center",
        va="bottom",
        fontsize=8,
        color="#e5e7eb"
...
def main():
    ...
    print("=" * 100)
    print("Step75F | Color-consistent before/after virtual perturbation spatial rescue atlas")
    print("=" * 100)
    ...
    outbase = outdir / "Fig_Step75F_BeforeAfterVirtualPerturbationSpatialRescueAtlas"
    ...
    value_path = outdir / "step75f_virtual_perturbation_spatial_values.csv"
    values.to_csv(value_path, index=False)

    perturb_audit_path = outdir / "step75f_perturbation_source_audit.csv"
    ...
    audit = {
        ...
        "interpretation_note": (
            "Step75F generates before/after virtual perturbation spatial maps. "
    ...
    (outdir / "step75f_report.json").write_text(json.dumps(audit, indent=2, ensure_ascii=False), encoding="utf-8")
    (outdir / "step75f_report.txt").write_text(json.dumps(audit, indent=2, ensure_ascii=False), encoding="utf-8")
